function PLOT_MODES(freestream_disturbances, A, s, chemistry, V, T_plot, w_infty)
...
% Part of: Hypersonics Stability MATLAB Solver - Postprocessing Module
    ...
    elseif s.shock.enabled && freestream_disturbances && T_plot == 0
        ...
        solution_old = s;
        s.shock.remesh_shock_distance = 1.5;
        s.mesh.Neta = s.mesh.Neta * 2;
        s.restart = true;
        disturbances = false;
    ...
    if T_plot > 0
        ...
        s = CFL_TIMESTEP(s);
        n_t = round(T_plot / s.time_integration.dt);
        dt  = T_plot / n_t;
        fprintf("\n")
    ...
    for i = 1:length(pert_plots)
        data      = pert_plots{i}{1};
        cb_label  = pert_plots{i}{2};
        filename  = sprintf('%s_T%.4f', pert_plots{i}{3}, T_plot);  % e.g. mode_pressure_T0.0100
        data_plot = real(data);
        CREATE_PUBLICATION_PLOT(x_coord, y_coord, data_plot, cb_label, ...
            s.shock.enabled, x_shock, y_shock, plot_params, s, filename);
    end
...
%  ========================================================================
function CREATE_PUBLICATION_PLOT(x, y, data, cb_label, has_shock, x_shock, y_shock, params, s, filename)
...
%       (none) - creates a new figure window.
    ...
    data_aspect = height_range / width_range / 1.3;
    ...
    hold on
    if has_shock
        plot(x_shock, y_shock, 'k-', 'LineWidth', params.line_width)
    end
    ...
    if isfield(s.running_plot, 'scaling_range')
    ...
    xlabel('$x/R$', 'FontSize', params.label_size, 'Interpreter', 'latex')
    ylabel('$y/R$', 'FontSize', params.label_size, 'Interpreter', 'latex')
    ...
    %% Set canvas size with explicit margins for all labels
    cb_margin     = 1.9;   % colorbar + label right
    left_margin   = 0.3;   % y-label left  
    bottom_margin = 1.0;   % x-label bottom
    top_margin    = 0.3;   % top tick/edge
    
    total_width  = plot_width  + cb_margin  + left_margin;
    total_height = plot_height + bottom_margin + top_margin;
    
    fig.Units         = 'inches';
    fig.Position      = [1, 1, total_width, total_height];
    fig.PaperUnits    = 'inches';
    fig.PaperSize     = [total_width, total_height];
    fig.PaperPosition = [0, 0, total_width, total_height];
    fig.PaperPositionMode = 'manual';
    
    ax_pos = [left_margin/total_width, ...          % left
          bottom_margin/total_height, ...        % bottom
          plot_width/total_width, ...            % width
          plot_height/total_height];             % height
...
%                                 freestream contributions added.
    ...
    if solution_remesh
        ...
        for k = 1:Nx
            pert_rho(  k, shock_idx(k):end) = 0;
            pert_rho_u(k, shock_idx(k):end) = 0;
            pert_rho_v(k, shock_idx(k):end) = 0;
            pert_rho_E(k, shock_idx(k):end) = 0;
        end
    end
