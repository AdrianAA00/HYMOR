function [chi, eta] = GO_TO_ELEMENT_SPACE(x, y, s)
...
% Part of: Hypersonics Stability MATLAB Solver - Mesh Module
    ...
    switch s.curvilinear_mapping.boundary_type
        ...
        case "circle"
            ...
            angle = mod(atan2(-x, y) + pi/2, 2*pi) - pi/2;
        ...
        case {"MSL", "blunt_cone"}
            ...
            for i = 1:size_chi
                ...
                elseif (-tan(pi/2 - s.curvilinear_mapping.theta) < tang) && (x_body(i,1) < 0)
                    ...
                    % Circular nose cap
                    angle = atan2((y_body(i,:) + s.curvilinear_mapping.R - s.curvilinear_mapping.L), x_body(i,:));
...
function s_inv = ldc_inverse(target, r, f_max)
...
%   F(s) = s + 4*(r-1)*(s^2/2 - s^3/3).
    ...
    s_inv = target;  % Initial guess (exact when r=1)

    for iter = 1:10
        F  = s_inv + 4*(r-1) * (s_inv.^2/2 - s_inv.^3/3);
        Fp = 1 + 4*(r-1) * s_inv .* (1 - s_inv);
