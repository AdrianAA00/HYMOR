...
#
#   Inputs:
#       v         (Nx x Ny) - Interior field values
#       s  (Dict{String,Any}) - Solution structure containing:
#           s["mesh"]["Nchi"], s["mesh"]["Neta"]  - Number of interior cells
#           s["shock"]["enabled"]                 - (Bool) Shock presence flag
#           s["shock"]["cell_indices"]            - (Nx x 1+) column indices of shocked cells
#
#   Outputs:
#       v_ext  ((Nx+2) x (Ny+2)) - Extended field with ghost cells
#
#   Notes:
...
function EXTEND_TO_GHOST_POINTS(v::AbstractMatrix{<:Number}, s::Dict{String,Any})

    Nx = s["mesh"]["Nchi"]
    Ny = s["mesh"]["Neta"]

    ## Initialize extended matrix (Nx+2) x (Ny+2)
    v_ext = zeros(eltype(v), Nx + 2, Ny + 2)
    ...
    if s["shock"]["enabled"]
        ...
        # Row indices (+1 for ghost offset)
        i_values_s = collect(1:Nx) .+ 1
        # Column indices of shocked cells
        j_values_s = s["shock"]["cell_indices"][:, 1]

        # Compute linear indices for shocked cells and their neighbours
        sz = size(v_ext)
        idx_0  = [LinearIndices(sz)[i_values_s[k], j_values_s[k] + 1] for k in 1:Nx]  # Shocked cell
        idx_m1 = [LinearIndices(sz)[i_values_s[k], j_values_s[k]]     for k in 1:Nx]   # One cell post-shock
        idx_m2 = [LinearIndices(sz)[i_values_s[k], j_values_s[k] - 1] for k in 1:Nx]   # Two cells post-shock
