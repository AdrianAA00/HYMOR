function FLUX_MOMENTUM(s::Dict{String, Any})
...
# using midpoint interpolation and midpoint quadrature.
#
# Part of: Hypersonics Stability Julia Solver - Operators Module
    ...
    @views temp_P_face = @. (p[1:end-1, 2:end-1] + p[2:end, 2:end-1]) * lr_xn / 2
    ...
    @views temp_P_face2 = @. (p[2:end-1, 1:end-1] + p[2:end-1, 2:end]) * bt_xn / 2
    ...
    @views temp_P_face = @. (p[1:end-1, 2:end-1] + p[2:end, 2:end-1]) * lr_yn / 2
    ...
    @views temp_P_face2 = @. (p[2:end-1, 1:end-1] + p[2:end-1, 2:end]) * bt_yn / 2
