function budgets_kinetic = COMPUTE_BUDGETS_KINETIC(V, s, output_flow)
...
%                     (rho, rho_u, rho_v, rho_E, p, mu, a, gamma_star), grid
%                     information (Nx, Ny, x, volume, flow_cells), freestream
%                     scaling factors, and dimension flag ("3D-axisymmetric"
%                     or "2D").
...
%
%   Outputs:
%       budgets_kinetic - (struct) Structure containing volume-integrated
%                         budget terms (suffixed _sum) and, optionally, the
%                         spatially resolved 2-D fields:
%           .A_adv_sum         - Integrated advection of kinetic energy.
%           .P_mom_sum         - Integrated momentum production.
%           .P_mass_sum        - Integrated mass (density) production.
%           .Dilat_P_sum       - Integrated pressure-dilatation.
%           .Transport_p_sum   - Integrated pressure transport.
%           .Transport_tau_sum - Integrated viscous transport.
%           .Dissipation_sum   - Integrated viscous dissipation.
%           .A_adv, .P_mom, .P_mass, .Dilat_P, .Transport_p,
%           .Transport_tau, .Dissipation - (Nx-by-Ny, only when output_flow
%                                           is true)
%
%   Notes:
%       - Perturbations are transformed to Chu variables (p', u', v', s')
%         via GET_CHU_COMPONENTS before budget evaluation.
%       - Viscosity is non-dimensionalised with rho_inf * U_inf.
%       - The viscous stress tensor assumes mu' is negligible (frozen
%         viscosity linearisation).
...
% Part of: Hypersonics Stability MATLAB Solver - Energy Budgets Module
    ...
    if s.PDE_dimension == "3D-axisymmetric"
        ...
        [dFlux_x_p_dx, ~]   = DERIVATIVE_EXT(Flux_x_p_Ext, s);
        [dFlux_x_tau_dx, ~]  = DERIVATIVE_EXT(Flux_x_tau_Ext, s);
        [~, dFlux_y_p_dy]    = DERIVATIVE_EXT(Flux_y_p_Ext, s);
        [~, dFlux_y_tau_dy]  = DERIVATIVE_EXT(Flux_y_tau_Ext, s);
    ...
    budgets_kinetic.A_adv_sum         = sum(A_adv         .* volume .* flow_cells, "all");
    budgets_kinetic.P_mom_sum         = sum(P_mom         .* volume .* flow_cells, "all");
    budgets_kinetic.P_mass_sum        = sum(P_mass        .* volume .* flow_cells, "all");
    budgets_kinetic.Dilat_P_sum       = sum(Dilat_P       .* volume .* flow_cells, "all");
    budgets_kinetic.Transport_p_sum   = sum(Transport_p   .* volume .* flow_cells, "all");
    budgets_kinetic.Transport_tau_sum = sum(Transport_tau  .* volume .* flow_cells, "all");
    budgets_kinetic.Dissipation_sum   = sum(Dissipation   .* volume .* flow_cells, "all");
