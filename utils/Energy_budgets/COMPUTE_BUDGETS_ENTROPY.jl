...
function COMPUTE_BUDGETS_ENTROPY(V, s, chemistry, output_flow)
    ...
    #       chemistry   - (Dict) Chemistry model providing eval_s(rho, e) for
    #                     entropy evaluation when chemistry_state is active.
    #       output_flow - (Bool) If true, the full 2-D field of every budget
    #                     term is stored in the output dict in addition to
    #                     the volume-integrated scalars.
    #
    #   Outputs:
    #       budgets_entropy - (Dict{String,Any}) containing volume-integrated
    ...
    if s["PDE_dimension"] == "3D-axisymmetric"
        ...
        (dFlux_entropy_x_dx, _) = DERIVATIVE_EXT(Flux_entropy_x_Ext, s)
        (_, dFlux_entropy_y_dy) = DERIVATIVE_EXT(Flux_entropy_y_Ext, s)
    ...
    budgets_entropy = Dict{String,Any}()
    budgets_entropy["A_adv_sum"]       = sum(A_adv       .* volume .* flow_cells)
    budgets_entropy["P_mom_sum"]       = sum(P_mom       .* volume .* flow_cells)
    budgets_entropy["P_mass_sum"]      = sum(P_mass      .* volume .* flow_cells)
    budgets_entropy["P_T_sum"]         = sum(P_T         .* volume .* flow_cells)
    budgets_entropy["Transport_sum"]   = sum(Transport   .* volume .* flow_cells)
    budgets_entropy["Dissipation_sum"] = sum(Dissipation .* volume .* flow_cells)
    budgets_entropy["Source_sum"]      = sum(Source      .* volume .* flow_cells)
