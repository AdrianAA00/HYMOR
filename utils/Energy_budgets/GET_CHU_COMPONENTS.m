function [pert_p, pert_u, pert_v, pert_entropy] = GET_CHU_COMPONENTS( ...
...
%       a_0          - (double array) Base-flow speed of sound (unused in
%                      the current formulation but retained for interface
%                      consistency with GET_ENERGY_NORM).
...
%
%   Outputs:
%       pert_p       - (double array) Perturbation pressure, computed as
%                      (gamma*-1) * (rho_E' - u_0*rho_u' - v_0*rho_v'
%                      + (u_0^2+v_0^2)/2 * rho').
%       pert_u       - (double array) Perturbation x-velocity,
%                      (rho_u' - u_0*rho') / rho_0.
%       pert_v       - (double array) Perturbation y-velocity,
%                      (rho_v' - v_0*rho') / rho_0.
%       pert_entropy - (double array) Non-dimensional entropy perturbation
%                      (s'/R), computed as p'/(p_0*(gamma*-1)) -
%                      rho'*gamma*/(rho_0*(gamma*-1)).
%
%   Notes:
