# START_SOLUTION  Build and initialize a fresh solution from scratch.
...
#
#   Inputs:
#       s         - (Dict) Solution struct with configuration parameters
#                   (Nchi, Neta, boundary_type, shock flag, etc.)
#       chemistry - (Dict) Chemistry model struct for thermodynamic evaluations
#
#   Outputs:
#       s         - (Dict) Fully initialized solution struct ready for time
...
function START_SOLUTION(s::Dict{String,Any}, chemistry::Dict{String,Any})

    ## Reset time and iteration counter
    s["time_integration"]["t"] = 0.0
